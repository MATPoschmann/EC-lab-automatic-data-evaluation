...
# plotofloops function is used to plot any values as scatter vs. the loopnumber
...
def Eatoverloop(datasheet, subfolder, title):
    plt.figure(figsize=(10,8))
    for EAT in ['EiR_at_5mA/cm2', 'EiR_at_10mA/cm2', 'EiR_at_50mA/cm2', 'EiR_at_100mA/cm2']:
        try:
            plt.scatter(datasheet['Loop'], datasheet[EAT])
        except:
    ...
    plt.ylabel('EiR /V')
    #legend
    plt.legend(['EiR_at_5mA/cm2', 'EiR_at_10mA/cm2', 'EiR_at_50mA/cm2', 'EiR_at_100mA/cm2'])
    #plotsaving
    new_filename = str(filename) + str(title)
    new_file_path = os.path.join(path, subfolder, new_filename)     
    plt.savefig(new_file_path, dpi = 300)
...
class CV():
    def __init__(self):
        ...
        # correcting the voltage of  Ewe by resistivity measured with PEIS and putting the values in new column called EiR
        self.CVcorrdf['EiR'] = self.CVdf['Ewe'] + RefElek - (self.CVdf['Re(Z)'] * self.CVdf['<I>']/1000)
        #reducing the number of columns to relevant  
        ...
        for loop in self.CVcorrcutdf['Loop'].unique():
            columnlist.append('Loop_' +str(int(loop))+'_EiR/V')
...
class ModularPotentio():
    def __init__(self):
        ...
        #correction of Ewe values and saving the EiR values in new column
        self.MPcorrdf['EiR'] = self.MPcorrdf['Ewe/V'] + RefElek - (self.MPcorrdf['Re(Z)'] * self.MPcorrdf['I/mA']/1000)
        ...
        # reducing number of columns to relevant
        self.MPcorrcutdf = self.MPcorrdf.loc[:,'EiR':'j/mA/cm^2'].drop(['I/mA'], axis = 1)
        ...
        for loop in self.MPcorrdf['Loop'].unique():
            ...
            for cycle in self.MPcorrdf['Ns'].unique():
                ...
                MPmeanEiR = MPmean['EiR'].iloc[-21:-1].mean()
        ...
        self.MPmeans = pd.DataFrame(MPmeans, columns = ['Loop', 'Cycle', 'EiR', 'j/mA/cm^2'])
        tafellist = []
        ...
        for loop in self.MPmeans['Loop'].unique():
            ...
            for value in range(len(tafelfit)-3):
                ...
                linear_model=np.polyfit(tafelfitcut['log(j/mA/cm^2)'], tafelfitcut['EiR'],1)
            ...
            plotandfit(self.tafelplot['log(j/mA/cm^2)'], self.tafelplot['EiR'], tafelcurve.iloc[0], tafelcurve.iloc[1], 'Loop '+ str(loop)+ ' Tafelslope: ' + str(tafelcurve.iloc[0].round(3)) + ' V/dec', 'MPevaluation','Tafelplot_loop_' + str(loop)+ '.png')            
        # making tafelplot data to DataFrame
        self.tafellist = pd.DataFrame(tafellist, columns = ['Loop', 'Tafelslope', 'y_at_x=0', 'EiR_at_5mA/cm2', 'EiR_at_10mA/cm2', 'EiR_at_50mA/cm2', 'EiR_at_100mA/cm2', 'EiR_at_10 mA/cm2 in dattarange'])   
        # ploting and saving every Eat value to a loop dependend graphic
        Eatoverloop(self.tafellist.dropna(axis=1), 'MPevaluation', '_EiRoverLoop')
...
class CP():
    def __init__(self):
        ...
        # correcting every Ewe-value using Re(Z) values and current and saving it as EiR-column
        self.CPcorrdf['EiR'] = self.CPdf['Ewe'] + RefElek - (self.CPdf['Re(Z)'] * self.CPdf['I']/1000)
        ...
        plotofloops(self.CPcorrcutdf, '_CPsvstime','CPevaluation', 'time', 'EiR', 't /s', 'EiR /V')
        ...
        for loop in self.CPcorrdf['Loop'].unique():
            ...
            Emean = df['EiR'].iloc[-51:-1].mean()
            EiRmean.append([loop, Emean])
        self.EiRmean = pd.DataFrame(EiRmean, columns = ['Loop', 'EiR_mean of last 50 values'])
        ...
        new_filename = str(filename) + "_EiRmean.csv"
        new_file_path = os.path.join(path,'CPevaluation', new_filename)
        self.EiRmean.round(5).to_csv(new_file_path, index = None, sep= ',')
        # plot and save mean-EiR-values vs Loops as graphic in subfolder
        plotsave(self.EiRmean['Loop'], self.EiRmean['EiR_mean of last 50 values'], 'CPevaluation', '_EiRmeanvsloop', 'Loop', 'EiR /V')
        return
...
RefElek = getvalue('Potential of reference electrode in V: ', float) # in V
